...
`pbi-context` is a local command-line tool and read-only [MCP](https://modelcontextprotocol.io) server
...
- **Read-only.** `pbi-context` never modifies your `.pbit`/`.pbip` source files — it only reads them and
...
| Version                      | Supported          |
| ----------------------------- | ------------------ |
| 1.1.x (`pbi-context`)         | :white_check_mark: |
| 1.0.x (published as `pbi-docs`) | :x: (renamed, see below) |
| < 1.0                         | :x:                |

This codebase's first published release was 2026-07-28, as `pbi-docs` on PyPI. It was renamed to
`pbi-context` on 2026-08-03 (same tool, name change only — see `CHANGELOG.md`); the `pbi-docs`
PyPI listing received one final `1.0.1` release pointing installers here and gets no further
updates. Security fixes are released as a new patch/minor version of `pbi-context` — there is no
long-term-support branch. Supported Python versions follow `pyproject.toml`'s classifiers
(currently 3.10–3.13).

## Reporting a Vulnerability

**Preferred: GitHub Private Vulnerability Reporting.** Use the **"Report a vulnerability"** button
under the [Security tab](https://github.com/Osc2405/pbi-context/security) of this repository
...
- The `pbi-context` version (`pbi-context --help` shows the version, or check `pip show pbi-context`) and
  Python version you're running.

### What to expect

`pbi-context` is maintained by a single person, so there's no formal SLA — but every report gets a
