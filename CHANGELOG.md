...
- **P1 follow-up to the same audit** (`Pruebas/auditoria_general_2026-07-24.md`), four items:
  ...
  - Suite: 265 tests, all green.

- **`extractor.py` (.pbit path) had zero test coverage** and **`cli.py --batch` had no tests at all** — flagged as the top-priority findings of a general architecture/test-coverage/tech-debt audit (`Pruebas/auditoria_general_2026-07-24.md`). Added `tests/test_extractor.py` (29 tests, in-memory zip fixtures built per-test — no `.pbit` fixture existed anywhere in the repo before) and `tests/test_cli_batch.py` (3 tests: no-match, all-succeed, partial-failure). Writing the extractor tests surfaced a real bug: `clean_json_text()`'s comment/trailing-comma regex substitutions used `r"\\1"` (literal backslash + "1") instead of `r"\1"` (group backreference), so cleaning a DataModelSchema containing `//`/`/* */` comments or trailing commas corrupted the JSON instead of fixing it, turning a recoverable parse into a `SchemaParseError`. Fixed in `extractor.py`. Suite: 258 tests, all green.
