---
agent: 'agent'
description: 'Analyze a Power BI model (.pbit, .pbip, .SemanticModel folder, or project root) using pbi-context, without leaving chat.'
---
...
```
pbi-context -i "${input:modelPath}" -o output
```

If `pbi-context` is not on PATH (not installed as a package), run it as a module instead, with the
working directory set to the pbi-context repo root — `pbi_extractor/cli.py` uses relative imports,
...
2. **For a specific table's columns/measures/DAX**, run
   `pbi-context --query output/<model-name> --table "TableName"` — returns one JSON object with a
...
3. **For one specific measure once you already know the table and measure name**, run
   `pbi-context --query output/<model-name> --table "TableName" --measure "MeasureName"` — cheaper
...
4. **For "which tables have X measures/columns" / "find the measure/column that does Y" questions**
   ...
   file by hand), run `pbi-context --query output/<model-name> --search-measures "keyword"` or
   `--search-columns "keyword"` (optionally `--category revenue` etc.).
5. **For relationship/join questions**, run
   `pbi-context --query output/<model-name> --relationships [--table "TableName"]`.
